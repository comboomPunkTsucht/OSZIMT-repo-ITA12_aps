...
const arm_exp_parser_1 = require("./arm-exp-parser");
const arm_parser_types_1 = require("./arm-parser-types");
class ARMParser {
    //
    // Create a new ARM Parser
    //
    constructor(iconBasePath, name, reporter, editor, cache) {
        // Both of these are overwritten when parse() is called
        this.template = { $schema: '', parameters: {}, variables: {}, resources: [] };
        this.expParser = new arm_exp_parser_1.default(this.template);
        this.elements = [];
    ...
    //
    parse(templateJSON, parameterJSON) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield this.processResources(this.template.resources);
            // Check for errors
            if (this.error) {
                throw this.error;
            }
    ...
    //
    preProcess(resources, parentRes) {
            ...
            for (const res of resources) {
                try {
                    ...
                    if (res.kind && res.type.includes('microsoft.web')) {
                        if (res.kind.toLowerCase().includes('api')) {
                            img = 'microsoft.web/apiapp.svg';
                        }
                        if (res.kind.toLowerCase().includes('mobile')) {
                            img = 'microsoft.web/mobileapp.svg';
                        }
                        if (res.kind.toLowerCase().includes('function')) {
                            img = 'microsoft.web/functionapp.svg';
                        }
                        if (res.kind.toLowerCase().includes('linux')) {
                            img = 'microsoft.web/serverfarmslinux.svg';
                        }
                    }
                    ...
                    if (res.type === 'microsoft.resources/deployments' && res.properties
                        ...
                        try {
                            if (this.cache) {
                                cacheResult = this.cache.get(linkUri);
                            }
                            if (cacheResult === undefined) {
                                ...
                                const fetchResult = yield utils.timeoutPromise(5000, fetch(linkUri), 'HTTP network timeout');
                                ...
                                subTemplate = yield fetchResult.text();
                            ...
                            if (this.editor) {
                                ...
                                const vscode = yield Promise.resolve().then(() => require('vscode')); // await on import! Voodoo!
                                ...
                                if (fileParentDir && fileName) {
                                    ...
                                    try {
                                        const fileContent = yield vscode.workspace.fs.readFile(vscode.Uri.parse(`${wsPath}/${fileParentDir}/${fileName}`));
                                ...
                                if (!subTemplate && fileName && wsLocalFile !== fileName) {
                                    ...
                                    if (wsLocalDir === '.') {
                                        search = `**/${fileName}`;
                                    } // Handle case where folder is at root of ws
                                    console.log(`### ArmView: That didn't work. So will search workspace for: ${search}`);
                                    // Try to run the search
                                    let searchResult;
                                    try {
                                        searchResult = yield vscode.workspace.findFiles(search);
                                        if (searchResult && searchResult.length > 0) {
                                            console.log(`### ArmView: Found & using file: ${searchResult[0]}`);
                                            const fileContent = yield vscode.workspace.fs.readFile(searchResult[0]);
                                            subTemplate = fileContent.toString();
                                        }
                                    }
                                    catch (err) {
                        ...
                        if (subTemplate) {
                            yield this.parseLinkedOrNested(res, subTemplate);
                        }
                        else {
                    ...
                    const cyNode = new arm_parser_types_1.CytoscapeNode('nodes');
                    cyNode.data = {
                        extra: extraData,
                        id: res.id,
                        img,
                    ...
                    if (res.dependsOn) {
                        res.dependsOn.forEach((dependsOn) => {
                    ...
                    if (res.resources) {
                        yield this.processResources(res.resources);
                    }
                }
                catch (err) {
                    this.error = err;
    ...
    //
    addLink(r1, r2) {
        const edge = new arm_parser_types_1.CytoscapeNode('edges');
        edge.data = {
            id: `${r1.id}_${r2.id}`,
            source: r1.id,
            target: r2.id,
    ...
    //
    parseLinkedOrNested(res, subTemplate) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (subTemplate) {
                ...
                try {
                    const linkRes = yield subParser.parse(subTemplate);
...
exports.default = ARMParser;
//# sourceMappingURL=arm-parser.js.map