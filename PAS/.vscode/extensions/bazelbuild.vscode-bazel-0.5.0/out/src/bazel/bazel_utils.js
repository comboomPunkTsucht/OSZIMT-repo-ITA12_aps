...
function getTargetsForBuildFile(bazelExecutable, workspace, buildFile) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const queryResult = yield new bazel_query_1.BazelQuery(bazelExecutable, workspace, `kind(rule, ${pkg}:all)`, []).queryTargets([], /* sortByRuleName: */ true);
        return queryResult;
    });
}
...
function shouldIgnorePath(fsPath) {
    ...
    const pathsToIgnore = bazelConfig.get("pathsToIgnore");
    for (const pathRegex of pathsToIgnore) {
        try {
            ...
            vscode.window.showErrorMessage("pathsToIgnore value isn't a valid regex: " + escape(pathRegex));
        }
    }
    return false;
}
