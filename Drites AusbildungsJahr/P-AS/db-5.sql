CREATE    DATABASE IF NOT EXISTS `db-5` DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_german2_ci;
...
CREATE    TABLE IF NOT EXISTS `reparatur_t` (
          ...
          CONSTRAINT `fk_reparatur_rechnungs_nr` FOREIGN KEY (`f_rechnungs_nr`) REFERENCES `rechnung_t` (`p_rechnungs_nr`) ON UPDATE CASCADE ON DELETE NO ACTION,
...
CREATE    TABLE IF NOT EXISTS `rechnung_t` (
          ...
          CONSTRAINT `fk_rechnung_material_nr` FOREIGN KEY (`f_material_nr`) REFERENCES `material_t` (`material_nr`) ON UPDATE CASCADE ON DELETE NO ACTION,
          CONSTRAINT `fk_rechnung_kunde_nr` FOREIGN KEY (`f_kunde_nr`) REFERENCES `kunden_t` (`p_kunden_nr`) ON UPDATE CASCADE ON DELETE NO ACTION
          ) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_german2_ci;

-- Generiere 100 Dummy-Datensätze für die Tabelle 'kunden_t'
INSERT    INTO `kunden_t` (
          `vname`  ,
          `nname`  ,
          `strasse`,
          `haus_nr`,
...
SELECT    CONCAT('R',f.id),
          CONCAT('M',f.id)
FROM      (
          SELECT    @rownum_f:=@rownum_f+1 AS id
          FROM      (
                    SELECT    0
                    UNION    
                    SELECT    1
...
SELECT    fk.id           ,
          CONCAT('R',fk.id)
FROM      (
          SELECT    @rownum_fk:=@rownum_fk+1 AS id
          FROM      (
                    SELECT    0
                    UNION    
                    SELECT    1
...
SELECT    CONCAT('M',m.id),
          CONCAT('H',m.id)
FROM      (
          SELECT    @rownum_m:=@rownum_m+1 AS id
          FROM      (
                    SELECT    0
                    UNION    
                    SELECT    1
...
SELECT    CONCAT('H',         h.id),
          ...
          CONCAT('VST',       h.id)
FROM      (
          SELECT    @rownum_h:=@rownum_h+1 AS id
          FROM      (
                    SELECT    0
                    UNION    
                    SELECT    1
...
SELECT    DATE_ADD(NOW(), INTERVAL r.id   DAY),
          RAND()*10                           ,
          DATE_ADD(NOW(), INTERVAL r.id+7 DAY),
          r.id                                ,
          CONCAT('R',                     r.id)
FROM      (
          SELECT    @rownum_r:=@rownum_r+1 AS id
          FROM      (
                    SELECT    0
                    UNION    
                    SELECT    1
...
SELECT    CONCAT('Material',mt.id),
          RAND()*100
FROM      (
          SELECT    @rownum_mt:=@rownum_mt+1 AS id
          FROM      (
                    SELECT    0
                    UNION    
                    SELECT    1
...
SELECT    rt.id,
          ...
          rt.id
FROM      (
          SELECT    @rownum_rt:=@rownum_rt+1 AS id
          FROM      (
                    SELECT    0
                    UNION    
                    SELECT    1
